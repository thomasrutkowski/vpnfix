...
printHelp ()
{
        ...
        echo  "\tOptions"
        echo  "\tprep\t - fixes racoon.conf. Run first and only once!!!"
        echo  "\t\t Creates directory /etc/racoon/remote/ \n"
        echo  "\tgetcfg\t - Get vpn config."
        echo  "\t\t Repeat for each VPN connection you have."
        echo  "\t\t NOTE! This will disconnect you! - Reconnect for new settings.\n"
        echo  "\tfix\t - Run after doing getcfg for all connections"
        echo  "\t\t Changes the config path in /etc/racoon/racoon.conf \n"
        echo  "\tunfix\t - Return to Apple original config."
        echo  "\t\t Run to enable grabbing new vpn tunnel settings with getcfg.\n"
